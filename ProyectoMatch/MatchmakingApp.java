...
/**
 * SIMULADOR DE SERVIDOR DE MATCHMAKING
 * Propósito: Gestionar perfiles y sesiones de juego usando Tablas Hash (HashMap).
 */
public class MatchmakingApp extends JFrame {
    ...
    private final Color COLOR_SECUNDARIO = Color.decode("#1f2933"), COLOR_TEXTO = Color.decode("#ece8e1");
    ...
    enum Estado { CONECTADO, EN_PARTIDA, DESCONECTADO }
    ...
    public MatchmakingApp() {
        ...
        new javax.swing.Timer(1000, e -> {
            ...
            if (panelEquipo != null) panelEquipo.repaint();
        }).start();
    ...
    public static String obtenerNombreRango(int elo) {
        if (elo < 300) return "HIERRO";
        if (elo < 600) return "BRONCE";
        if (elo < 900) return "PLATA";
        if (elo < 1200) return "ORO";
        if (elo < 1500) return "PLATINO";
        if (elo < 1800) return "DIAMANTE";
        if (elo < 2100) return "ASCENDENTE";
        if (elo < 2400) return "INMORTAL";
        return "RADIANTE";
    }

    public static Color obtenerColorRango(int elo) {
        if (elo < 300) return Color.GRAY;
        if (elo < 1200) return Color.YELLOW;
        if (elo < 1800) return Color.CYAN;
        if (elo < 2400) return Color.MAGENTA;
        return Color.ORANGE;
    }
    ...
    private void finalizarPartida() {
        if (tablaSesiones.isEmpty()) return;
    ...
    private JPanel crearPantallaLogin() {
        ...
        gbc.gridy = 1; login.add(Box.createVerticalStrut(200), gbc); // Espacio para el dibujo
        gbc.gridy = 2; login.add(txt, gbc);
        gbc.gridy = 3; login.add(btn, gbc);
        return login;
    }
    ...
    private JPanel crearPantallaPrincipal() {
        ...
        header.add(btnReg); header.add(btnMatch); header.add(btnEstado); header.add(btnFinalizar);
        ...
        btnEstado.addActionListener(e -> {
            ...
            if(yo != null) yo.estado = (yo.estado == Estado.CONECTADO) ? Estado.EN_PARTIDA : Estado.CONECTADO;
        });
    ...
    static class Jugador {
        ...
        public Jugador(String n, String e, int elo) { this.nombre = n; this.email = e; this.elo = elo; }
    }

    static class Partida { 
        String id; List<Jugador> jugadores; 
        public Partida(String id, List<Jugador> j) { this.id = id; this.jugadores = j; } 
    }
    ...
    private JButton crearBoton(String t, Color c) {
        ...
        b.setFocusPainted(false); b.setFont(new Font("SansSerif", Font.BOLD, 10));
        return b;
    }

    class TarjetaAgente extends JPanel {
        Jugador j;
        public TarjetaAgente(Jugador j) { this.j = j; setOpaque(false); }
